...
import { EXTERNAL_TOOLS, isExternalTool } from '../../shared/tools'
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.toolsInstall, (_event, tool: unknown) => {
    if (!isExternalTool(tool)) throw new Error(`Unknown tool: ${String(tool)}`)
    return installTool(tool)
  })

  ipcMain.handle(IPC_CHANNELS.toolsRemove, (_event, tool: unknown) => {
    if (!isExternalTool(tool)) throw new Error(`Unknown tool: ${String(tool)}`)
    return removeTool(tool)
  })

  ipcMain.handle(IPC_CHANNELS.toolsInstallable, () =>
    Object.fromEntries(
      EXTERNAL_TOOLS.map((tool) => [tool, whyNotInstallable(tool)]).filter(
        ([, why]) => why !== null
      )
    )
  )
